...
class ObjectNode(Node):
    ...
    def emit_verbose(self):
        return {"_type": self.constructor, "_args": [arg.emit() for arg in self.args]}
...
class DictAdapter(BaseAdapter):
    ...
    def build_node(self, obj, context):
        return DictNode({str(key): context.build_node(val) for key, val in obj.items()})
...
class AdapterRegistry:
    ...
    def register(self, *args, **kwargs):
        if len(args) == 2 and not kwargs:
            ...
            if not isinstance(adapter, BaseAdapter):
                raise TypeError(
                    "register expected a BaseAdapter instance, got %r" % adapter
                )
    ...
    def js_context_class(self):
        return type(
            "JSContext",
            (self.js_context_base_class,),
            {"registry": self, "telepath_js_path": self.telepath_js_path},
        )
